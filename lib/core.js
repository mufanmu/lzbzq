...
export const TIMELINE_MAX = 50;// 时间线最多保留条数
export const EVENTS_MAX = 40;  // 弹幕事件最多保留条数

export const TIERS = [
  { name: '梁神',       value: 100, emoji: '😇', color: '#c2853b' },
  { name: '梁圣',       value: 83,  emoji: '🙏', color: '#cc785c' },
  { name: '梁文锋叔叔', value: 67,  emoji: '🧧', color: '#b0703c' },
  { name: '梁子',       value: 50,  emoji: '😐', color: '#8e8b82' },
  { name: '牢梁',       value: 33,  emoji: '🕳', color: '#5f6b7a' },
  { name: '小难梁',     value: 17,  emoji: '🥀', color: '#5a4a42' },
  { name: '梁西皮',     value: 0,   emoji: '🖤', color: '#3d3a35' }
];
 ...
 * lzbzq:timeline         list（JSON 记录，最新在前）
...
export async function ensureNewDay(store, now) {
  ...
  if (state.lastDate !== today) {
    const prevTier = tierOf(state.startValue).name;   // 先取旧档位再覆盖
...
export async function computeState(store, ip, now) {
  ...
  const timeline = (rawTimeline || []).map((s) => { try { return JSON.parse(s); } catch (e) { return null; } }).filter(Boolean);
  const rawEvents = await store.lrange('lzbzq:events', 0, EVENTS_MAX - 1);
  const recentVotes = (rawEvents || []).map((s) => { try { return JSON.parse(s); } catch (e) { return null; } }).filter(Boolean).reverse();
