...
export const MAX_SETTLE_DAYS = 90; // 跨天回放结算上限（防止异常数据导致回放过长）
...
/** timeline 中是否已有某日期的总结（幂等检查：防并发/重复结算写两条） */
async function timelineHas(store, date) {
  const raw = await store.lrange('lzbzq:timeline', 0, TIMELINE_MAX - 1);
  return (raw || []).some((x) => {
    const r = parseJsonSafe(x);
    return !!(r && r.date === date);
  });
}

/** 惰性跨天结算（自愈版）：从 state.lastDate 起逐日回放结算到今天
 *  - state 丢失时：扫描遗留 lzbzq:votes:*，从最早有票日回放，历史票不再变孤儿
 *  - 多天未访问时：中间每天逐日结算（原实现只结算 lastDate 一天，会漏中间日）
 *  - 无票日跳过（不写总结、startValue 不动）；总结按日期幂等写入
 */
export async function ensureNewDay(store, now) {
  const today = beijingDateStr(now);
  let state = await store.getJSON('lzbzq:state');
  if (!state) {
    // 自愈：先找遗留的投票数据，从最早有票日的前一天开始回放
    const voteKeys = await store.scanKeys('lzbzq:votes:*');
    const dates = voteKeys
      .map((k) => k.slice('lzbzq:votes:'.length))
      .filter((d) => /^\d{4}-\d{2}-\d{2}$/.test(d))
      .sort();
    const lastDate = dates.length ? addDays(dates[0], -1) : today;
    state = { lastDate: lastDate >= today ? today : lastDate, startValue: START_VALUE };
    if (state.lastDate >= today) {
      await store.setJSON('lzbzq:state', state);   // 冷启动（无历史可回放）立即落库，避免每请求重复扫描
    }
  }
  if (state.lastDate < today) {
    let d = state.lastDate;
    for (let i = 0; i < MAX_SETTLE_DAYS && d < today; i++) {
      const votes = expandVotes(await store.hgetall('lzbzq:votes:' + d));
      if (votes.length) {
        const rec = settleDay(d, votes, state.startValue);
        state.startValue = rec.end;                     // 链式继承：无票日不动
        // 幂等：SETNX 独占该日总结写入权（并发跨天只写一条）；timelineHas 兜底兼容已写入（如补偿脚本）
        const claimed = await store.setnx('lzbzq:settled:' + d, '1', 7 * 86400);
        if (claimed && !(await timelineHas(store, d))) {
    ...
    state.lastDate = d;   // 正常完成时 d === today；超过回放上限时停在截断点，下次继续结算
...
export function createMemoryStore() {
  ...
  return {
    ...
    async scanKeys(pattern) {
      ...
      return [...m.keys()].filter((k) => re.test(k));
    },
    async setnx(k, v, ttlSec) {   // ttlSec 内存版忽略
      if (m.has(k)) return null;  // 已存在 → 未抢占
      m.set(k, String(v));
      return 'OK';
    },
    _raw: m
